...
namespace JustSaying.FlexiblePublishers.Queued
{
    public interface IQueuedMessagePublisher : IMessagePublisher
    {
        ...
        Task PublishAsync(Message message, bool isWhitelisted, CancellationToken cancellationToken = default(CancellationToken));

        int QueuedItems { get; }

        Task ProcessQueueAsync(bool onlySendWhitelisted, CancellationToken cancellationToken = default(CancellationToken));
    }
}
